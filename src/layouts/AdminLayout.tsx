    ...
    >
      ...
      {isActive && (
        <motion.div
          ...
          className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-gradient-to-b from-violet-500 to-purple-500 rounded-full"
      ...
      {isActive && <ChevronRight className="h-4 w-4 text-violet-500" />}
    </motion.button>
  );
};
...
export const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    { title: t('nav.dashboard'), icon: LayoutDashboard, href: '/admin/dashboard' },
    { title: t('nav.users'), icon: Users, href: '/admin/users' },
    { title: t('nav.doctors'), icon: Stethoscope, href: '/admin/doctors' },
    { title: t('nav.specializations'), icon: BookOpen, href: '/admin/specializations' },
    { title: t('nav.doctorRequests'), icon: Shield, href: '/admin/doctor-requests' },
    { title: t('nav.bookings'), icon: Calendar, href: '/admin/bookings' },
    { title: t('nav.symptomChecker'), icon: Activity, href: '/admin/symptom-checker', badge: 'AI' },
  ];

  // Mobile bottom nav items
  const mobileNavItems = [
    { title: t('nav.home') || 'Home', icon: LayoutDashboard, href: '/admin/dashboard' },
    { title: t('nav.users') || 'Users', icon: Users, href: '/admin/users' },
    { title: t('nav.doctors') || 'Doctors', icon: Stethoscope, href: '/admin/doctors' },
    { title: t('nav.bookings') || 'Bookings', icon: Calendar, href: '/admin/bookings' },
    { title: t('nav.more') || 'More', icon: Menu, href: '/admin/specializations' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      ...
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-violet-50/50 via-transparent to-transparent pointer-events-none" />

      {/* Mobile header - Admin style */}
          ...
          <div className="flex items-center justify-between h-14 px-4">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-xs text-gray-500">{t('portal.admin') || 'Admin Portal'}</p>
            ...
            <div className="flex items-center gap-1">
              ...
              <motion.div whileTap={{ scale: 0.9 }}>
                <Button variant="ghost" size="icon" className="relative w-10 h-10 rounded-full">
                  ...
                  <span className="absolute top-2 right-2 w-2 h-2 bg-violet-500 rounded-full animate-pulse" />
                </Button>
              </motion.div>
            </div>
          </div>
          ...
          <div className="h-16 flex items-center justify-between px-5 border-b border-gray-100">
            <Link to="/" className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400 -mt-0.5">{t('portal.admin')}</p>
              </div>
            </Link>
          </div>
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 p-4 text-white">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-semibold">{t('sidebar.adminTools') || 'Admin Tools'}</span>
                </div>
                <p className="text-xs text-white/80 mb-3">{t('sidebar.adminToolsDesc') || 'Access system settings and configurations'}</p>
                <Button
                  size="sm"
                  className="w-full bg-white text-violet-600 hover:bg-white/90 font-medium shadow-lg"
                  onClick={() => navigate('/admin/specializations')}
                >
                  {t('sidebar.configure') || 'Configure'}
                </Button>
              </div>
            </div>
          </div>
        ...
        <header className="hidden lg:flex h-16 items-center justify-between px-6 border-b border-gray-100 bg-white/50 backdrop-blur-sm sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" className="relative">
              ...
              <span className="absolute top-2 right-2 h-2 w-2 bg-violet-500 rounded-full animate-pulse" />
            </Button>
      ...
      <MobileBottomNav items={mobileNavItems} accentColor="violet" />
