...
import { ArrowRight, HeartPulse, Loader2, Stethoscope } from 'lucide-react';
...
export const DoctorLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user.is_doctor) {
        setError(t('auth.accessDenied.doctor'));
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-background theme-doctor">
      ...
      <div className="fixed inset-0 bg-gradient-to-br from-teal-50/80 via-transparent to-cyan-50/50" />
        ...
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
            ...
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold text-gradient-doctor">{t('auth.doctor.title')}</h1>
              <p className="text-muted-foreground mt-2">
                {t('auth.doctor.subtitle')}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="email"
                  type="email"
                  placeholder={t('placeholders.doctorEmail')}
                  value={email}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Button variant="outline" size="sm" asChild className="h-10">
                <Link to="/admin/login">{t('portal.admin')}</Link>
              </Button>
            </div>
          </div>
        </div>
